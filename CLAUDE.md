...
**Tests** live in [src/tests/](src/tests/) (435 tests, all pass without API keys via mocks and Fernet fixtures). [src/tests/test_contracts.py](src/tests/test_contracts.py) is the regression suite for past pilot failures — every new incident class should add a contract here. The proactive agent's brainstorm/critique/judge loop is covered by `test_proactive_*` modules. The legacy root-level `tests/` directory was deleted in commit `bb776bd`; don't recreate it. `pytest` config is in `pyproject.toml`.
...
- **Use-case routing**: per turn, the LLM router also emits a use-case tag (`grievance` / `finance` / `finance_advice` / `scheme_kb` / `general`). The matching block from [src/bhai/llm/prompts/use_cases/](src/bhai/llm/prompts/use_cases/) gets injected into the system prompt. `finance_advice` (commit `5760959`) is distinct from `finance` — it fires when the user asks a math-shaped question (EMI/total interest/savings rate) and pulls in a math-discipline block. Add a new use-case = add a file there + extend `VALID_USE_CASES`.
...
- **Proactive agent loop (NEW, `src/bhai/proactive/`)**: a brainstorm → critique → tools → draft → judge pipeline (`ProactiveThinker`) that generates context-aware nudges. Currently runs via `scripts/run_proactive_dry_run.py` for offline evaluation; not yet wired into the production `inference/webhooks/nudges.py` loop. See `tmp/v2_proactive_design.md` for design.
