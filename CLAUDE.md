...
**Tests** live in [src/tests/](src/tests/) (567 tests, all pass without API keys via mocks and Fernet fixtures). [src/tests/test_contracts.py](src/tests/test_contracts.py) is the regression suite for past pilot failures — every new incident class should add a contract here. The proactive agent's brainstorm/critique/judge loop is covered by `test_proactive_*` modules. The legacy root-level `tests/` directory was deleted in commit `bb776bd`; don't recreate it. `pytest` config is in `pyproject.toml`.

**Branches**: `main` is prod (auto-deploys to Railway on push). `dev` is integration (deploys to the dev bot). Branch protection requires CI + 1 approval. Don't push to `main` directly. (Note: `.github/workflows/ci.yml` still triggers on `develop`, a dead branch — see the docs follow-up.)
...
- **The FAQ short-circuit was removed (commit `cd5c113`).** Every reply now runs through the main LLM with KB content scoped by [LLMKBRouter](src/bhai/llm/llm_router.py) (Sonnet 4.6 since `9ad1f63`, was Haiku — file was renamed from `haiku_router.py`). The router also reads recent conversation context to disambiguate follow-up questions. Don't revive `resilience/faq_cache.py`.
- **Prompts are file-based and switched by `PROMPT_VERSION` env var.** Active everywhere: `prompt_v1_pilot` ([src/bhai/llm/prompts/prompt_v1_pilot.md](src/bhai/llm/prompts/prompt_v1_pilot.md)) — also the code default (`config.prompt_version`). The old `current.md` was deleted; there's only one persona prompt now. Language mirroring (any of 11 Sarvam languages) lives ONLY here — don't duplicate it into use-case blocks.
...
- **`ESCALATE: true` actually sends an email now** ([src/bhai/escalations/handler.py](src/bhai/escalations/handler.py) via Gmail API, not Resend, not SMTP — Railway blocks outbound SMTP). The LLM also emits an `ESCALATE_CATEGORY`; `_recipients_for_category` routes it: `docs_bc`→Priti, `docs_midc`→Dinesh, `docs_unknown`→Anu (triage), `workplace`→Simran (HR), `mental_health`→Rishi, `loan_hardship`→Priti. Sundar (`escalation_cc`) is CC'd on **every** email; Anu (`escalation_impact_head`) is CC'd on the impact-team categories (`docs_*`, `mental_health`, `loan_hardship`) but NOT `workplace` (HR is outside the impact team). Categories live in `_ESCALATION_CATEGORIES` ([src/bhai/llm/base.py](src/bhai/llm/base.py)). When proposing prompt changes that touch ESCALATE wording, also consider the consent flow in `prompt_v1_pilot.md`.
...
- **Use-case routing**: per turn, the LLM router also emits a use-case tag (`grievance` / `finance_advice` / `scheme_kb` / `general`). The matching block from [src/bhai/llm/prompts/use_cases/](src/bhai/llm/prompts/use_cases/) gets injected into the system prompt. The old `finance` tag (and `finance.md`) were dropped; `finance_advice` fires when the user asks a math-shaped question (EMI/total interest/savings rate) and pulls in a math-discipline block. **Helpdesk KB is injected only on `scheme_kb` turns** — the other tags carry no KB, which is what keeps non-scheme turns fast. Add a new use-case = add a file there + extend `VALID_USE_CASES`.
- **Multilingual TTS (commit `5dc11a8`)**: `detect_language_code()` in [src/bhai/tts/sarvam_tts.py](src/bhai/tts/sarvam_tts.py) picks the right `target_language_code` per call by counting characters in 9 Indic Unicode blocks (Devanagari, Bengali, Gurmukhi, Gujarati, Odia, Tamil, Telugu, Kannada, Malayalam). Before this, all replies (Tamil/Telugu/Bengali) went through Sarvam's Hindi voice and mispronounced everything. The currency normalization only runs for Devanagari (hi-IN/mr-IN); other scripts handle currency natively.
- **Reactive multimodal & web (`<image>` / `<map>` blocks + web_search)**: the LLM can emit `<image>...</image>` (poster/diagram brief) and `<map>...</map>` (place → Google Maps link) blocks inside its reply, and run Anthropic's server-side `web_search` for current facts. The Telegram webhook ([inference/webhooks/telegram_webhook.py](inference/webhooks/telegram_webhook.py)) strips these blocks from the TTS voice and delivers them as separate messages: `_extract_image_brief` → Gemini image gen (`nanobanana`, `gemini-2.5-flash-image`) → `send_photo`; `_extract_map_links` → link text. Gated by `WEB_SEARCH_ENABLED` + a Google/Gemini API key. The proactive draft path strips the same blocks.
- **Proactive agent loop (`src/bhai/proactive/`)**: a brainstorm → critique → tools → draft → judge pipeline (`ProactiveThinker`) that generates context-aware nudges. It IS the production nudge generator now — `_generate_nudge_v2` in [inference/webhooks/nudges.py](inference/webhooks/nudges.py) calls it. Three slots/day: **morning + night** are light relational check-ins (`think_checkin`, no tools/artifacts); **afternoon** is the substantive utility output (`think_substantive`, the full brainstorm→judge loop). Night may close on a joke only if the day's mood was playful. Gated by `NUDGE_ENABLED` (kill-switch, default off) + `NUDGE_PHONES` (`*` = all active). `scripts/run_proactive_dry_run.py` still runs it offline. See `tmp/v2_proactive_design.md`.
...
- **When you catch a KB-staleness issue, email Anu.** During accuracy audits (or any pre-deploy spot-check), if you discover the KB has stale or wrong info — a scheme has been paused, a fee has changed, a centre has moved, a phone number is dead, two sections of the same file contradict — surface it to Anu (anu@tinymiracles.com), CC'ing Sundar (`sundar@tinymiracles.com` — NEVER `sundar91101@gmail.com`). Keep emails crisp: name the file, quote the wrong text, link the source for the correct info, suggest the edit. Remind Anu she can edit the KB herself via git — clone the repo, edit `knowledge_base/helpdesk/*.md`, branch, PR to `dev`, Sundar tests on the dev bot, merges to `main`, Railway auto-deploys. **Do NOT offer to open the PR for her** — the impact team needs to learn git themselves, and doing it for them is a learning-anti-pattern (Sundar's call 2026-05-27). Offer support on individual steps if she asks; never the whole flow. If multiple KB items need updating in one session, batch them into ONE email rather than spamming. Use the Gmail MCP tool (`mcp__claude_ai_Gmail__create_draft`) to draft — the auto-mode classifier will gate the first one per session for user approval, which is the right default. Also: BEFORE drafting any KB-staleness email, WebFetch the primary source you're citing to confirm the claim — WebSearch summaries aggregate multiple sources and can be wrong; the email is going to a real person who'll act on it, so verify the specific claim before sending.
